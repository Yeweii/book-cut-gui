...
def binarize_otsu_from_array(arr: np.ndarray) -> Image.Image:
...
def binarize_otsu(image: Image.Image) -> Image.Image:
    """Otsu 全局阈值二值化。"""
    return binarize_otsu_from_array(_to_gray_array(image))
...
def binarize_adaptive(
    ...
    return binarize_adaptive_from_array(_to_gray_array(image), block_size=block_size, C=C)
...
def binarize_sauvola(
    ...
    return binarize_sauvola_from_array(
        _to_gray_array(image), window_size=window_size, k=k, R=R
    )
